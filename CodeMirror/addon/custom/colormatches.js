...
(function(mod) {
	...
	var	colors = require('./definedcolors');
	...
	function destroyHighlighter(cm) {
			if (cm._colorHighlighter) {
...
.h-phantom:before{content:attr(data-color);color:inherit;background-color:inherit;pointer-events:none;position:absolute;top:0;left:0;border-radius:2px;white-space:pre;}";
	...
	//var Colorhighlighter = require('./colorhighlighter.js');
	
	CodeMirror.defineOption("colorhighlighter", false, function(cm, val, old) {
		
		
		function processEditor(editor) {
			var cm = editor;
			if (cm) {
				cm.__hasColorHighlighterListeners = true;
				addHighlighter(cm);	
			}else{
				destroyHighlighter(cm);
			}
		}
